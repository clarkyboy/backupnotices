...
$startD = date("Y-m-d", strtotime(trim($_POST['startdate'])));
$endD = date("Y-m-d", strtotime(trim($_POST['enddate'])));
$ISPName = trim($_POST['ISPName']);
$relatedtitleID = trim($_POST['relatedtitleID']);

echo '<script>alert("'.$startD.'");</script>';
echo '<script>alert("'.$endD.'");</script>';
echo '<script>alert("'.$ISPName.'");</script>';
echo '<script>alert("'.$relatedtitleID.'");</script>';
    ...
    if($ISPName == "All"){
			...
			if($data){
				...
				if($hasdl){
					...
					echo '<script>alert("No Such Files Existed!");</script>';
				}
			...
			if($data){
				...
				if($hasdl){
					...
					echo '<script>alert("No Such File Existed!");</script>';
				}}
	...
	if($ISPName == "All"){
		if($status == "All"){
			...
			if($data){
				...
				else{echo '<script>alert("No Such File Existed!");</script>';}
			}//end of foreach isp
		}//end of status All
		else{
			$country = isset($_POST['country'])? $_POST['country'] : '';
...
if($ISPName == "All"){
	...
	while($ctr < $limit){
		...
		foreach($datum as $tableRow){
			...
			}//end of foreach $datum
				
		}//end of while ($ctr < $limit)
	...
	// $sheet++;
		...
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
				...
				foreach($isp as $isps){
						...
						echo '<script>alert("'.$ispname.'");</script>';
						echo '<script>alert("'.$excelfilename.'");</script>';
						echo '<script>alert("'.$zipfilename.'");</script>';
...
foreach($datum as $key=>$tableRow){
	...
	echo '<script>alert("No Transaction!");</script>';
}
