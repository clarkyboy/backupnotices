...
?>
    ...
    <?php
    	...
    	{
    		if (preg_match("/([a-z]).*([a-z])/i", $search, $m, PREG_OFFSET_CAPTURE)) 
    		  {
				$first = $m[0][1];
				$last = $m[count($m)-1][1];
				//print "First: $first, last: $last";
				$NoticeID = substr($search,$last+1);
			  }
        ...
        if($notices->search_notice_id($NoticeID))
        {
    ?>
           ...
           <table id="dg" title="Notices Per IP" class="easyui-datagrid" style="height:600px;"
            ...
            <tbody>
               <?php 
                   foreach($notices->search_notice_id($NoticeID) as $s)
                    {
                        echo '<tr>';
